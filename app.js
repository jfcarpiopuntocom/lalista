...
let filterState = { status:"all", category:"all", q:"" };
...
function seedDemo(){
  ...
  const demoPhones = ["+81 90 1234 5678","+34 612 345 678","+593 99 123 4567","+1 415 555 0182","+852 9123 4567","+49 151 2345 6789"];
  ["Ryo Tanaka","Selene Vox","Marco Cruz","Ana Kade","Iris Chen","Damian Reyes"].forEach((n,i)=>{
    db.guests.push({
      id:uid(), eventId:ev1, name:n, phone:demoPhones[i],
      vip: i%3===0, category: i%3===0?"VIP":(i===2?"Backstage":"General"), plusOnes: i%2, status: i===0?"checked-in": i===1?"confirmed":"pending",
...
function toast(msg, type=""){
  const t = el("div",{class:"toast "+type}, [
    el("span",{},type==="success"?"✓":type==="danger"?"⚠":type==="warn"?"⏳":"◈"),
    el("span",{},msg)
  ]);
...
function setActiveEvent(id){
  DB.activeEventId = id; saveDB();
  pageState.page = 1; selectedIds.clear();
  render();
  sfx("click");
}
...
function guestsForActiveEvent(){
  ...
  if(filterState.category && filterState.category!=="all") list = list.filter(g=>(g.category||"General")===filterState.category);
  if(filterState.q){
...
/* ---- Teléfono internacional (intl-tel-input): sin país fijo, detecta por IP/navegador ---- */
let itiInstance = null;
function initPhoneWidget(existingE164){
  const input = $("#g-phone");
  if(itiInstance){ itiInstance.destroy(); itiInstance=null; }
  itiInstance = window.intlTelInput(input, {
    initialCountry: "auto",
    geoIpLookup: cb=>{
      // servicio gratuito de geo-IP para sugerir país por defecto (el usuario puede cambiarlo libremente)
      fetch("https://ipapi.co/json").then(r=>r.json()).then(d=> cb(d && d.country_code ? d.country_code.toLowerCase() : "us"))
        .catch(()=> cb("us"));
    },
    preferredCountries: [],
  ...
  if(existingE164){
    try{ itiInstance.setNumber(existingE164); }catch(e){ input.value = existingE164; }
  }
  const status = $("#g-phone-status");
  function validate(){
    const val = input.value.trim();
    if(!val){ status.textContent=""; status.className="phone-status"; return true; }
    const ok = itiInstance.isValidNumber();
    status.textContent = ok? "✓ Número válido ("+ (itiInstance.getSelectedCountryData().name||"") +")" : "✗ Número inválido para el país seleccionado";
    status.className = "phone-status "+(ok?"ok":"bad");
    return ok || !val;
  }
  input.addEventListener("blur", validate);
  input.addEventListener("countrychange", validate);
  initPhoneWidget._validate = validate;
}

/* ===== Historial de invitado recurrente (cross-evento, por teléfono) ===== */
function guestHistory(phone, excludeId){
  if(!phone) return null;
  const matches = DB.guests.filter(g=> g.phone===phone && g.id!==excludeId);
  if(!matches.length) return null;
  const events = new Set(matches.map(g=>g.eventId));
  const checkedIns = matches.filter(g=>g.status==="checked-in").length;
  return { visits: events.size, checkedIns };
}
function refreshHistoryBadge(){
  const phone = itiInstance ? (itiInstance.getNumber ? itiInstance.getNumber() : "") : "";
  const excludeId = $("#g-id").value;
  const hist = phone && itiInstance.isValidNumber() ? guestHistory(phone, excludeId) : null;
  const field = $("#g-history-field"), badge = $("#g-history-badge");
  if(hist){
    badge.textContent = `🔁 Ya registrado en ${hist.visits} evento(s) de este club` + (hist.checkedIns? ` · ${hist.checkedIns} check-in(s) previos`:"");
...
function openGuestModal(guest){
  ...
  $("#g-category").value = guest? (guest.category||"General") : "General";
  ...
  $("#g-history-field").style.display = "none";
  openModal("#modal-guest");
  initPhoneWidget(guest? guest.phone : "");
  const phoneInput = $("#g-phone");
  phoneInput.addEventListener("blur", refreshHistoryBadge);
  refreshHistoryBadge();
}
$("#g-save").addEventListener("click", ()=>{
  ...
  let phoneE164 = "";
  if($("#g-phone").value.trim()){
    if(itiInstance && !itiInstance.isValidNumber()){
      toast("El teléfono no es válido para el país seleccionado","danger"); sfx("error");
      initPhoneWidget._validate && initPhoneWidget._validate();
      return;
    }
    phoneE164 = itiInstance ? itiInstance.getNumber() : $("#g-phone").value.trim();
  }
  const id = $("#g-id").value;
  const category = $("#g-category").value;
  const payload = {
    name, phone: phoneE164, plusOnes: Number($("#g-plusones").value)||0,
    category, vip: category==="VIP" || category==="Backstage",
    notes: $("#g-notes").value.trim(), addedBy: $("#g-addedby").value
  };
...
function guestPayload(g, ev){
  return JSON.stringify({app:"MLDI", v:1, code:g.code, eventId:ev.id, guestId:g.id, name:g.name, category:g.category||"General"});
}
...
function handleSyncScan(raw){
  try{
    ...
    // merge invitados: gana el registro con status más "avanzado" — con resumen de conflictos
    ...
    data.guests.forEach(incoming=>{
      ...
      if(!local){
        DB.guests.push(incoming); added++;
      } else if(rank[incoming.status] > rank[local.status]){
        details.push({name: incoming.name, from: local.status, to: incoming.status});
        Object.assign(local, incoming); updated++;
      } else {
        skipped++;
      }
    });
    ...
    saveDB(); render();
    $("#scan-result").innerHTML = `✓ Sincronizado «${escapeHTML(data.event.name)}»`;
    sfx("success");
    if(window.confetti) confetti({particleCount:60,spread:80,origin:{y:.3}});
    setTimeout(()=>{ closeModal("#modal-scan"); showSyncSummary(added, updated, skipped, details); }, 700);
  }catch(e){
    $("#scan-result").innerHTML = "✗ QR de sincronización no válido.";
    sfx("error"); restartScanSoon();
  }
}
function showSyncSummary(added, updated, skipped, details){
  const html = `
    <div class="modal">
      <div class="modal-head"><h3>Resumen de sincronización</h3><button class="modal-close" id="sync-sum-close">×</button></div>
      <div class="modal-body">
        <div class="stats-strip" style="margin-bottom:14px;">
          ${statBoxHTML(added,"Agregados")}
          ${statBoxHTML(updated,"Actualizados")}
          ${statBoxHTML(skipped,"Sin cambios")}
        </div>
        ${details.length? `<div class="divider-label">Conflictos resueltos</div>
          <div style="max-height:180px; overflow-y:auto;">
          ${details.map(d=>`<div class="hint" style="margin-bottom:5px;">«${escapeHTML(d.name)}» : ${d.from} → <span style="color:var(--accent)">${d.to}</span></div>`).join("")}
          </div>` : `<p class="hint">Sin conflictos — todo ya estaba al día o se agregó directo.</p>`}
      </div>
    </div>`;
  const ov = el("div",{class:"overlay open", id:"modal-sync-summary", html});
  document.body.appendChild(ov);
  $("#sync-sum-close").addEventListener("click", ()=> ov.remove());
  ov.addEventListener("click", e=>{ if(e.target===ov) ov.remove(); });
  toast(`Sync: ${added} agregados, ${updated} actualizados, ${skipped} sin cambios`, "success");
}
function statBoxHTML(num,label){
...
$("#btn-export-csv").addEventListener("click", ()=>{
  ...
  const rows = [["nombre","telefono","categoria","acompanantes","estado","notas","codigo"]];
  DB.guests.filter(g=>g.eventId===ev.id).forEach(g=>{
    rows.push([g.name, g.phone||"", g.category||"General", g.plusOnes, g.status, (g.notes||"").replace(/,/g," "), g.code]);
  });
...
$("#file-csv").addEventListener("change", e=>{
  ...
  reader.onload = ()=>{
    ...
    lines.slice(1).forEach(line=>{
      ...
      const [name, phone, category, plusOnes] = clean;
      if(!name) return;
      const cat = ["General","VIP","Backstage"].includes(category) ? category : "General";
      DB.guests.push({id:uid(), eventId:ev.id, name:name.trim(), phone:(phone||"").trim(),
        category:cat, vip: cat==="VIP"||cat==="Backstage", plusOnes:Number(plusOnes)||0, status:"pending",
...
/* ===================== 12c. BACKUP / RESTORE COMPLETO (JSON) ===================== */
$("#btn-backup").addEventListener("click", ()=>{
  const blob = new Blob([JSON.stringify(DB,null,2)],{type:"application/json"});
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "lalista_backup_"+new Date().toISOString().slice(0,10)+".json";
  a.click();
  toast("Backup completo descargado","success");
});
$("#btn-restore").addEventListener("click", ()=> $("#file-json").click());
$("#file-json").addEventListener("change", e=>{
  ...
  reader.onload = ()=>{
    try{
      ...
      if(!data.events || !data.guests){ throw new Error("formato inválido"); }
      if(!confirm("Esto reemplazará TODOS los datos actuales (eventos, invitados, equipo). ¿Continuar?")) return;
      const prevDB = JSON.parse(JSON.stringify(DB));
      DB = Object.assign(defaultDB(), data);
      saveDB(); render();
      toast("Backup restaurado","success");
      pushUndo("restaurar backup anterior", ()=>{ DB = prevDB; saveDB(); });
    }catch(err){
      toast("Archivo de backup inválido","danger"); sfx("error");
    }
    e.target.value = "";
  };
  reader.readAsText(file);
...
/* ===================== 12d. AVISO DE EXPIRACIÓN PRÓXIMA (watcher) ===================== */
const expiryWarned = new Set();
function checkExpiryWarnings(){
  DB.events.forEach(ev=>{
    const st = eventStatus(ev);
    if(st==="soon" && !expiryWarned.has(ev.id)){
      expiryWarned.add(ev.id);
      toast(`⏳ «${ev.name}» expira en menos de 24h`, "warn");
    }
    if(st!=="soon") expiryWarned.delete(ev.id);
  });
}
...
setInterval(()=>{ render(true); checkExpiryWarnings(); }, 30000);
  ...
  main.appendChild(el("div",{class:"toolbar"},[
    el("div",{class:"search-box"},[
      ...
      el("input",{id:"search-input", placeholder:"Buscar por nombre, teléfono o nota…  (Ctrl+K)", value:filterState.q,
        oninput:e=>{ filterState.q = e.target.value; pageState.page=1; selectedIds.clear(); renderGuestTable(); }})
    ]),
    el("div",{class:"chip-filter"}, statuses.map(([k,label])=>
      el("button",{class:filterState.status===k?"active":"", onclick:()=>{filterState.status=k; pageState.page=1; selectedIds.clear(); renderGuestTable(); refreshFilterChips();}}, label)
    )),
    (()=>{
      const s = el("select",{id:"filter-category", style:"width:auto; padding:8px 10px;", onchange:e=>{
        filterState.category = e.target.value; pageState.page=1; selectedIds.clear(); renderGuestTable();
      }},[
        el("option",{value:"all"},"Todas las categorías"),
        el("option",{value:"General"},"General"),
        el("option",{value:"VIP"},"VIP"),
        el("option",{value:"Backstage"},"Backstage"),
      ]);
      s.value = filterState.category;
      return s;
    })(),
    el("button",{class:"btn btn-ghost btn-icon", id:"btn-density", title:"Densidad de tabla",
      onclick:()=>toggleDensity()}, DB.settings.density==="compact"?"☰":"☰☰")
  ]));

  // ===== LA LISTA — foco central =====
  main.appendChild(el("div",{class:"guest-table-wrap"+(DB.settings.density==="compact"?" density-compact":""), id:"guest-table-wrap"}));
  main.appendChild(el("div",{class:"pager", id:"guest-pager"}));
  renderGuestTable();
...
function toggleDensity(){
  DB.settings.density = DB.settings.density==="compact" ? "comfortable" : "compact";
  saveDB(); render(); sfx("click");
}

function renderGuestTable(){
  const wrap = $("#guest-table-wrap");
  if(!wrap) return;
  wrap.className = "guest-table-wrap"+(DB.settings.density==="compact"?" density-compact":"");
  const all = guestsForActiveEvent();
  ...
  const pageGuests = all.slice(start, start+pageState.size);
  ...
  const table = el("table",{},[
    el("thead",{},[ el("tr",{}, [
      el("th",{style:"width:32px;"}, el("input",{type:"checkbox", checked:allPageSelected?"checked":null,
        onchange:e=>{
          if(e.target.checked) pageIds.forEach(id=>selectedIds.add(id));
          else pageIds.forEach(id=>selectedIds.delete(id));
          renderGuestTable();
        }}))
    ].concat(cols.map(([k,label])=>
    ...
    el("tbody",{}, pageGuests.map(g=> guestRow(g)))
  ]);
...
function renderPager(total, totalPages){
  const pager = $("#guest-pager");
  if(!pager) return;
  if(total<=pageState.size){ pager.innerHTML=""; return; }
  pager.innerHTML="";
  pager.appendChild(el("div",{class:"pager-bar"},[
    el("button",{class:"btn btn-ghost btn-sm", disabled: pageState.page<=1?"disabled":null,
      onclick:()=>{ pageState.page--; renderGuestTable(); }}, "‹ Anterior"),
    el("span",{class:"pager-info"}, `Página ${pageState.page} de ${totalPages}  ·  ${total} invitados`),
    el("button",{class:"btn btn-ghost btn-sm", disabled: pageState.page>=totalPages?"disabled":null,
      onclick:()=>{ pageState.page++; renderGuestTable(); }}, "Siguiente ›")
  ]));
}

/* ===== Bulk actions bar ===== */
function renderBulkBar(allFiltered){
  let bar = $("#bulk-bar");
  if(!selectedIds.size){ if(bar) bar.remove(); return; }
  if(!bar){
    bar = el("div",{id:"bulk-bar", class:"bulk-bar"});
    document.body.appendChild(bar);
  }
  bar.innerHTML = "";
  bar.appendChild(el("span",{class:"bulk-count"}, `${selectedIds.size} seleccionado(s)`));
  if(allFiltered.length > selectedIds.size){
    bar.appendChild(el("button",{class:"btn btn-ghost btn-sm", onclick:()=>{
      allFiltered.forEach(g=>selectedIds.add(g.id)); renderGuestTable();
    }}, `Seleccionar los ${allFiltered.length} filtrados`));
  }
  bar.appendChild(el("button",{class:"btn btn-solid btn-sm", onclick:()=>bulkSetStatus("checked-in")}, "✓ Check-in"));
  bar.appendChild(el("button",{class:"btn btn-ghost btn-sm", onclick:()=>bulkSetStatus("no-show")}, "✕ No-show"));
  bar.appendChild(el("button",{class:"btn btn-danger btn-sm", onclick:()=>bulkDelete()}, "🗑 Eliminar"));
  bar.appendChild(el("button",{class:"btn btn-ghost btn-sm", onclick:()=>{ selectedIds.clear(); renderGuestTable(); }}, "Cancelar"));
}
function bulkSetStatus(status){
  const ids = Array.from(selectedIds);
  const prev = ids.map(id=>{
    const g = DB.guests.find(x=>x.id===id);
    return {id, status:g.status, checkedInAt:g.checkedInAt};
  });
  ids.forEach(id=>{
    const g = DB.guests.find(x=>x.id===id);
    g.status = status; g.checkedInAt = status==="checked-in" ? Date.now() : null;
  });
  saveDB(); selectedIds.clear(); render();
  if(status==="checked-in") sfx("checkin");
  toast(`${ids.length} invitados actualizados a "${status}"`, "success");
  pushUndo(`cambio masivo de ${ids.length} invitados a ${status}`, ()=>{
    prev.forEach(p=>{ const g=DB.guests.find(x=>x.id===p.id); if(g){ g.status=p.status; g.checkedInAt=p.checkedInAt; } });
    saveDB();
  });
}
function bulkDelete(){
  const ids = Array.from(selectedIds);
  if(!confirm(`¿Eliminar ${ids.length} invitados seleccionados?`)) return;
  const removed = DB.guests.filter(g=>ids.includes(g.id));
  DB.guests = DB.guests.filter(g=>!ids.includes(g.id));
  saveDB(); selectedIds.clear(); render();
  toast(`${removed.length} invitados eliminados`, "danger");
  pushUndo(`eliminar ${removed.length} invitados`, ()=>{ DB.guests.push(...removed); saveDB(); });
}
function guestRow(g){
  const staff = DB.team.find(t=>t.id===g.addedBy);
  const tr = el("tr",{class:g.status==="checked-in"?"checked-in":(g.status==="no-show"?"no-show":"")},[
    el("td",{},[ el("input",{type:"checkbox", checked:selectedIds.has(g.id)?"checked":null,
      onchange:e=>{ if(e.target.checked) selectedIds.add(g.id); else selectedIds.delete(g.id); renderBulkBar(guestsForActiveEvent()); } }) ]),
    el("td",{},[
      el("div",{class:"g-name"},[g.name, g.category && g.category!=="General"? el("span",{class:"cat-tag cat-"+g.category}, g.category):null]),
